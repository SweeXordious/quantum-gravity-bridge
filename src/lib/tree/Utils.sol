...
function getStartingBit(uint256 numLeaves) private pure returns (uint256 startingBit) {
...
// solhint-disable-next-line func-visibility
function pathLengthFromKey(uint256 key, uint256 numLeaves) private pure returns (uint256 pathLength) {
    if (numLeaves <= 1) {
